...
func RegisterDenoms() {
	if err := sdk.RegisterDenom("fury", sdkmath.LegacyOneDec()); err != nil {
		panic(err)
	}

	if err := sdk.RegisterDenom("afury", sdkmath.LegacyNewDecWithPrec(1, accounts.EtherDecimals)); err != nil {
		panic(err)
	}
}
