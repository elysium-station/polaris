...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	jq '.app_state["mint"]["params"]["mint_denom"]="afury"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.consensus["params"]["block"]["max_gas"]="30000000"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

	# Allocate genesis accounts (cosmos formatted addresses)
	for KEY in "${KEYS[@]}"; do
		./bin/polard genesis add-genesis-account $KEY 100000000000000000000000000afury --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	./bin/polard genesis add-genesis-account black1yrene6g2zwjttemf0c65fscg8w8c55w5vhc9hd 69000000000000000000000000afury --keyring-backend $KEYRING --home "$HOMEDIR"

	# Sign genesis transaction
	./bin/polard genesis gentx ${KEYS[0]} 1000000000000000000000afury --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
...
./bin/polard start --pruning=nothing "$TRACE" --log_level $LOGLEVEL --api.enabled-unsafe-cors --api.enable --api.swagger --minimum-gas-prices=0.0001afury --home "$HOMEDIR"